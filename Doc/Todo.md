...
[ ] TD-012 Reconcile `Doc/TimeDesign.md` RTC-read guidance with the actual `TimeReadCmos` implementation, or strengthen the implementation to match the documented double-read approach.
...
[ ] TD-014 Clarify in `Doc/TimeDesign.md` that the current wall-time model interpolates time-of-day between resyncs but does not currently advance the calendar date independently of RTC resync.
