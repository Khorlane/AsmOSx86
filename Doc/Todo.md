...
[x] TD-008 Replace the `Doc/CallGraph.md` placeholder with a current call graph or a scoped initial version.
[x] TD-009 Decide whether wall time should remain lazily initialized or be explicitly initialized during kernel boot.
[x] TD-010 Refactor `Uptime.asm` to follow the project memory-based interface model instead of register-based contracts and `pusha`/`popa` preservation.
