...
;     TimeDay/TimeMon/TimeYear = current wall calendar date.
;   Notes:
;     Lazily calls TimeSync if wall time has not been initialized.
;     Resyncs from CMOS after TIME_RSYNC_SEC seconds of monotonic time.
;     Carries midnight rollover into the calendar date.
;--------------------------------------------------------------------------------------------------
TimeNow:
...
;   Output:
;     WallSecDay = seconds since midnight from the CMOS wall clock.
;     WallSyncLo/WallSyncHi = monotonic tick baseline for resync policy.
;     WallLastLo/WallLastHi = last monotonic tick used for wall advancement.
;     WallRemTicks = 0.
;     WallSyncValid = 1.
;--------------------------------------------------------------------------------------------------
TimeSync:
